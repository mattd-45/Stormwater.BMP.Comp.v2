// City input adapters registry + default normalizer.
// Exposes:
//   - window.CityAdapterBase.createProjectInputs(cityUiState, cityRules)
//   - window.CityAdapters[cityKey].adapt(cityUiState, cityRules)
//   - window.adaptCityInputs(cityKey, cityUiState, cityRules) -> { inputs, warnings }

(function (global) {
  const CityAdapterBase = {
    createProjectInputs(cityUiState, cityRules) {
      const s = cityUiState || {};

      const inputs = {
        areas: {
          perviousLandscapingUsable: Number(s.perviousLandscapingUsable) || 0,
          imperviousVehicularPavement: Number(s.imperviousVehicularPavement) || 0,
          imperviousPedestrianPavement: Number(s.imperviousPedestrianPavement) || 0,
          perviousTreeCoverNonUsable: Number(s.perviousTreeCoverNonUsable) || 0,
          flatDeckOnStructureArea: Number(s.flatDeckOnStructureArea) || 0,
          slopedRoofArea: Number(s.slopedRoofArea) || 0,
          paversOnStructureArea: Number(s.paversOnStructureArea) || 0,
          imperviousCAUntreated: Number(s.imperviousCAUntreated) || 0
        },
        targets: {
          detentionCF: Number(s.targetDetentionCF) || 0,
          retentionCF: Number(s.targetRetentionCF) || 0,
          detentionNeeded: !!s.detentionNeeded,
          retentionNeeded: !!s.retentionNeeded
        },
        constraints: {
        ...
        assumptions: {
          ...
          programmableSpaceIsHighValue: !!s.programmableSpaceIsHighValue,
          allowSteepSlopeGreenRoof: !!s.allowSteepSlopeGreenRoof
        },
        flags: {
          // reserved for future per-city booleans / switches
        }
      };
      ...
      if (roofArea <= 0) {
        warnings.push('No rooftop or plaza areas entered; rooftop BMP options may be unavailable.');
      }

      if (inputs.targets.detentionNeeded && !inputs.targets.detentionCF) {
        warnings.push('Detention target is marked as needed but CF value is 0.');
      }
      if (inputs.targets.retentionNeeded && !inputs.targets.retentionCF) {
        warnings.push('Retention target is marked as needed but CF value is 0.');
      }
  ...
  const registry = global.CityAdapters || {};

  function adaptCityInputs(cityKey, cityUiState, cityRules) {
    const entry = registry[cityKey];
    if (entry && typeof entry.adapt === 'function') {
      return entry.adapt(cityUiState, cityRules);
    }
    // Fallback: generic mapping for any city without a custom adapter.
    return CityAdapterBase.createProjectInputs(cityUiState, cityRules);
  }

  global.CityAdapterBase = CityAdapterBase;
  global.CityAdapters = registry;
